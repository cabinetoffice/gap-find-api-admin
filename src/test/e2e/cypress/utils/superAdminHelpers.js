...
function checkFirst10RowsContent(today) {
  for (let i = 0; i < 10; i++) {
    ...
    cy.get(`[data-cy="admin-dashboard-list-table-row-API-key-${keyName}"]`).should('have.text', keyName);
    cy.get(`[data-cy="admin-dashboard-list-table-row-Department-${keyName}"]`).should('have.text', 'Test Org');
    cy.get(`[data-cy="admin-dashboard-list-table-row-Created-${keyName}"]`).should('have.text', today);
    cy.get(`[data-cy="admin-dashboard-list-table-row-Revoked-${keyName}-link"]`).should('contain', 'Revoke');
  }
}
...
function createKeyForFundingOrganisation1(baseUrl, startingPoint, endingPoint) {
  ...
  for (let i = startingPoint; i < endingPoint; i++) {
    ...
    cy.visit(`${baseUrl}/api-keys/create`);
...
function createKeyForFundingOrganisation2(baseUrl, startingPoint, endingPoint) {
