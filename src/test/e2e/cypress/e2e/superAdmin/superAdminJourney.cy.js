...
describe('Super Admin Journey', () => {
  ...
  it('should show only the filtered keys when department filters are set, and clear all filters should reset those filter', () => {
    ...
    cy.url().should(
      'eq',
      `${BASE_URL}/api-keys/manage?selectedDepartments=Test+Org`
    );
    cy.get(`[data-cy="admin-dashboard-filter-Test Org-checkbox"]`).should(
      'be.checked'
    );
    ...
    cy.url().should(
      'eq',
      `${BASE_URL}/api-keys/manage?selectedDepartments=Evil+Org&selectedDepartments=Test+Org`
    );
    //rows
    cy.get(`[data-cy="admin-dashboard-list-table-body"]`)
      .should('be.visible')
  ...
  it('Should throw exception when revoking API key', () => {
    ...
    cy.get('[data-cy="error-back-link"]')
      .should('have.attr', 'href', '/find/api/admin/api-keys/manage')
