...
describe('Technical support and Admin Roles User journey ', () => {
  ...
  describe('API key dashboard without keys', () => {
    it('Should render API key dashboard, display no API keys, header and footer', () => {
      ...
      cy.get('[data-cy="footer"]')
        .should('be.visible')
        .contains('All content is available under the Open Government Licence v3.0, except where otherwise stated');

      cy.get('[data-cy="api-keys-heading"]').should('be.visible').contains('Manage API keys');

      cy.get('[data-cy="api-keys-department"]').should('be.visible').contains('Department');

      cy.get('[data-cy="api-keys-department-name"]').should('be.visible').contains('Test Org');
      ...
      cy.get('[data-cy="api-keys-create-button"]').should('be.visible').contains('Create an API key');
    });
  });
  ...
  describe('Create an API Key', () => {
    ...
    it('Should render create an API key page', () => {
      cy.visit(`${BASE_URL}/api-keys`);
      cy.get('[data-cy="api-keys-create-button"]').should('be.visible').contains('Create an API key').click();
      ...
      cy.get('[data-cy="create-key-heading"]').should('be.visible').contains('Name your API key');
      ...
      cy.get('[data-cy="create-key-continue"]').should('be.visible').contains('Continue');

      cy.get('[data-cy="create-key-back-button"]').should('be.visible').should('have.text', 'Back').click();

      cy.get('[data-cy="api-keys-heading"]').should('be.visible').should('have.text', 'Manage API keys');
    ...
    it('Should display validation errors for blank name', () => {
      cy.get('[data-cy="create-key-continue"]').should('be.visible').contains('Continue').click();

      cy.get('[data-cy="create-key-error-banner-heading"]').should('be.visible').contains('There is a problem');

      cy.get('[data-cy="create-key-error-summary-list"]').should('have.length', 1).contains('Enter a key name');
    ...
    it('Should successfully create key and display value', () => {
      ...
      cy.get('[data-cy="new-key-heading"]').should('be.visible').should('have.text', 'New API key');

      cy.get('[data-cy="new-key-your-api-key"]').should('be.visible').should('have.text', 'Your API key:');
      ...
      cy.get('[data-cy="new-key-paragraph"]')
        .should('be.visible')
        .should('have.text', 'You should keep a copy of this API key somewhere safe.');

      cy.get('[data-cy="new-key-paragraph-2"]')
        .should('be.visible')
        .should('have.text', 'When you leave this screen, you will not be able to see this API key again.');

      cy.get('[data-cy="new-key-back-button"]').should('be.visible').contains('Back to your API keys').click();
  ...
  describe('API key dashboard with keys', () => {
    it('Should render API key dashboard with list of API keys', () => {
      ...
      cy.get('[data-cy="api-keys-department-name"]').should('be.visible').should('have.text', 'Test Org');

      cy.get('[data-cy="create-key-summary-list"]').children().should('have.length', 1);

      cy.get(`[data-cy="api-key-name-${apiKeyName}"]`).should('be.visible').should('have.text', apiKeyName);

      cy.get(`[data-cy="api-key-created-date-${apiKeyName}"]`)
        .should('be.visible')
        .should('have.text', 'Created ' + today);

      cy.get(`[data-cy="api-key-revoke-${apiKeyName}"]`).should('be.visible').contains('Revoke');
  ...
  describe('Revoke API key', () => {
    it('Should render API key confirmation page and revoke API key', () => {
      ...
      cy.get(`[data-cy="api-key-revoke-${apiKeyName}"]`).should('be.visible').contains('Revoke').click();

      cy.get('[data-cy="revoke-heading"]').should('be.visible').should('have.text', 'Revoke an API key');
      ...
      cy.get('[data-cy="revoke-revoke-button"]').should('be.visible').should('have.text', 'Revoke key');

      cy.get('[data-cy="revoke-cancel-button"]').should('be.visible').should('have.text', 'Cancel').click();

      cy.get(`[data-cy="api-key-revoke-${apiKeyName}"]`).should('be.visible').contains('Revoke').click();

      cy.get('[data-cy="revoke-revoke-button"]').should('be.visible').should('have.text', 'Revoke key').click();
  ...
  describe('Error page', () => {
    it('Should render error page when trying to access super admin dashboard', () => {
      ...
      cy.get('[data-cy="error-heading"]').should('be.visible').should('have.text', 'Something went wrong');

      cy.get('[data-cy="error-paragraph"]')
        .should('be.visible')
        .should('have.text', 'Something went wrong while trying to complete your request.');
...
//main difference between an user with only Technical Support Role vs an user with Technical Support and Admin Role, is that the first will not have any navBar
describe('Technical support User Journey', () => {
  ...
  describe('API key dashboard without keys', () => {
    it('Should render API key dashboard, display no API keys, header and footer', () => {
      ...
      cy.get('[data-cy="header-navbar"]').should('not.exist');
      ...
      cy.get('[data-cy="api-keys-department-name"]').should('be.visible').contains('Evil Org');
  ...
  describe('API key dashboard with keys', () => {
    it('Should render API key dashboard with list of API keys', () => {
      ...
      cy.get('[data-cy="api-keys-department-name"]').should('be.visible').should('have.text', 'Evil Org');
