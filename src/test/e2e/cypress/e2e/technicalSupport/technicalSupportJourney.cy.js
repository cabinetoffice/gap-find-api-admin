describe('API key dashboard without keys', () => {
  ...
  it('Should render API key dashboard, display no API keys, header and footer', () => {
    ...
    cy.get('[data-cy="beta-banner"]')
      .should('be.visible')
      .contains('BETA');

    cy.get('[data-cy="header-feedback-link"]')
      .should('have.attr', 'href', 'https://docs.google.com/forms/d/e/1FAIpQLSd2V0IqOMpb2_yQnz_Ges0WCYFnDOTxZpF299gePV1j8kMdLA/viewform')
      .contains('feedback');
    ...
    cy.get('[data-cy="api-keys-no-api-keys"]')
      .should('be.visible')
      .contains('You don\'t have any API keys yet. When you create one, you will be able to see a list of your API keys and revoke access to them on this screen.');
...
describe('Create an API Key', () => {
  ...
  it('Should render create an API key page', () => {
    ...
    cy.get('[data-cy="create-key-hint-text"]')
      .should('be.visible')
      .contains('This name will be shown in your list of API keys to make it easier to find. We recommend using the name of the service you will be using the API key to integrate with');
    ...
    cy.url().should('include', '/admin/api-keys'); // TODO maybe remove this
...
//    TODO : test copy to clipboard
      ...
      cy.get('[data-cy="new-key-paragraph"]')
                 .should('be.visible')
                 .contains('You should keep a copy of this API key somewhere safe.');

      cy.get('[data-cy="new-key-paragraph-2"]')
                 .should('be.visible')
                 .contains('When you leave this screen, you will not be able to see this API key again.');

      cy.get('[data-cy="new-key-back-to-api-keys"]')
            .should('be.visible')
            .click();

      cy.url().should('include', '/admin/api-keys'); // TODO maybe remove
...
describe('API key dashboard with keys', () => {
  ...
  it('Should render API key dashboard with list of API keys', () => {
    ...
    cy.get('[data-cy="api-key-created-date-apiKeyName"]')
      .should('be.visible')
      .contains('Created'); // TODO created date
...
describe('Revoke API key', () => {
  ...
  it('Should render API key confirmation page', () => {
    cy.visit('http://localhost:8084/find/api/admin/api-keys');
    ...
    cy.get('[data-cy="revoke-heading"]')
      .should('be.visible')
      .contains('Revoke an API key');

    cy.get('[data-cy="revoke-paragraph-1"]')
      .should('be.visible')
      .contains('If you revoke this API key, you will no longer be able to use it to request data from Find a grant.');

    cy.get('[data-cy="revoke-paragraph-2"]')
      .should('be.visible')
      .contains('You cannot re-enable a revoked API key. If you want to request data again, you will need to create a new API key.');
...
describe('Error page', () => {
  ...
  it('Should render error page when trying to access super admin dashboard', () => {
    ...
    cy.get('[data-cy="error-paragraph"]')
      .should('be.visible')
      .contains('Something went wrong while trying to complete your request.');
